...
The Form version XML as a nested list, or the raw response if `parse` is
`FALSE`.

## Details

To get the XML of a published Form version, this function requests the
version resource with `.xml` appended. Since the XForms specification
allows blank strings as versions, pass the special value `___` (three
underscores) as the version to retrieve the blank version.

## See also

<https://docs.getodk.org/central-api-form-management/#retrieving-form-version-xml>
