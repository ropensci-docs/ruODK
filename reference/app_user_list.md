...
A tibble with one row for each App User of the Project as per the ODK
Central API docs. Column names are renamed from ODK's `camelCase` to
`snake_case`.

## Details

App Users may only be created, fetched, and manipulated within the
nested Projects subresource, as App Users themselves are limited to the
Project in which they are created.

## See also

<https://docs.getodk.org/central-api-accounts-and-users/#listing-all-app-users>
