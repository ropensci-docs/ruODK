...
A tibble with one row per expected Draft attachment and the columns
`name`, `type`, `exists`, `blob_exists`, `dataset_exists` and
`updated_at`, as far as Central returns them. Column names are renamed
from ODK Central's `camelCase` to `snake_case`.

## Details

When a Draft is created, the expected Form Attachments are computed and
slots are created, as with a new Form. Attachments that match existing
ones on the published Form are copied over to the new Draft.
