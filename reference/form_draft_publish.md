...
- version:
  ...
  (character) The version to associate with the Draft once it is
  published. Default: `NULL` (the Draft keeps its version).
...
definition (and attachments). If the Draft version conflicts with an
older version of the Form, the publish operation fails, unless the new
version string is given with `version` and Central sets it on publish.
Once the Draft is published, there is no longer a Draft version of the
Form. Publishing a Draft Form that defines a Dataset schema also
