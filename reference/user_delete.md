...
all existing sessions. The User record remains on file within the
database, so that when for example information about the creator of a
Form or Submission is requested, basic details are still available on
file. A new User account may be created with the same email address as
any deleted account.

## See also

<https://docs.getodk.org/central-api-accounts-and-users/#deleting-a-user>
