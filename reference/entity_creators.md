...
A tibble with one row per creating Actor and all Actor metadata as
columns as per the ODK Central API docs. Column names are renamed from
ODK Central's `camelCase` to `snake_case`.

## Details

This endpoint returns a list of all Actors who have created Entities in
the Dataset, sorted by display name.

## See also

<https://docs.getodk.org/central-api-entity-management/#entities-creators>
