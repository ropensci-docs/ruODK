...
`name` and its `properties` with `inForm` flags.

## Details

This endpoint lists the name and Properties of a Dataset that are
affected by a Form. The list of Properties includes all published
Properties of that Dataset, but each Property has the `inForm` flag to
note whether or not that Form fills it in.

## See also

<https://docs.getodk.org/central-api-form-management/#published-form-related-datasets>
