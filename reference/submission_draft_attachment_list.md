...
A tibble with one row per expected attachment and the columns `name` and
`exists`.

## Details

When a Draft Submission is created, its XML data determines which file
attachments it references. This lists the expected attachments with a
boolean flag telling whether the server holds a copy of each file or
not.
