...
- file:
  ...
  (character) Path to a local file whose bytes are uploaded to the
  expected file slot.

- content_type:

  (character) The MIME type sent as `Content-Type` header, e.g.
  `"image/jpeg"`. Default: `NULL`, which sends
...
endpoint. The expected file slots are determined by the Submission XML;
list them with
[`attachment_list()`](https://docs.ropensci.org/ruODK/reference/attachment_list.md).
`filename` must match a slot name from the attachment listing. `file` is
the local file. Its bytes fill the slot.

## See also

<https://docs.getodk.org/central-api-submission-management/#uploading-an-attachment>
