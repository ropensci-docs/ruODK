...
- display_name:
  ...
  (character) The friendly nickname of the App User to create.
...
A tibble with one row holding the new App User's metadata as columns,
including the `token` to authenticate requests as this App User. The
token is only returned at creation time.

## Details

The only information required to create a new App User is its display
name (called "Nickname" in the administration panel). When an App User
is created, it is assigned no rights. To grant rights, assign the App
User a Form Role with
[`form_assignment_grant()`](https://docs.ropensci.org/ruODK/reference/form_assignment_grant.md).

## See also

<https://docs.getodk.org/central-api-accounts-and-users/#creating-a-new-app-user>
