...
A tibble with one row holding the Form version's metadata as columns as
per the ODK Central API docs. Column names are renamed from ODK
Central's `camelCase` to `snake_case`.
...
special value `___` (three underscores) as the version to retrieve the
blank version.

## See also

<https://docs.getodk.org/central-api-form-management/#getting-form-version-details>
