...
- filter:
  ...
  by. Default: `NULL` (not sent).

- limit:

  (numeric) The maximum number of Submissions to extract geoinformation
  from. Default: `NULL` (not sent).

- fieldpath:

  (character) The field to extract geoinformation from. At most one
  field can be given. Default: `NULL` (the first geofield outside any
  repeat group).
...
A list with the GeoJSON `FeatureCollection` as returned by Central:
`type` and `features` with `geometry` and `properties` per Submission.

## Details

This endpoint returns Submissions' geodata, one Form field per
Submission, in GeoJSON format. If no particular field path is supplied,
the first non-repeat geopoint, geotrace or geoshape field is used. No
particular ordering is implied. An OData-style `$filter` query filters
the Submissions.

## See also

<https://docs.getodk.org/central-api-submission-management/#retrieving-submissions-geodata>
