...
- old_password:
  ...
  (character) The User's current password.

- new_password:

  (character) The new password the User wishes to set.
...
To directly update a User password, the User's intention must be
reproven by supplying the old password alongside the new password. To
initiate an email-based password reset process instead, see
[`user_reset_password()`](https://docs.ropensci.org/ruODK/reference/user_reset_password.md).

## See also

<https://docs.getodk.org/central-api-accounts-and-users/#directly-updating-a-user-password>
