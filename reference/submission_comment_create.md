...
A list with the new comment's `body` and the commenting Actor's
`actor_id`.

## Details

The only accepted data is `body`, which contains the body of the comment
to be made. It is not possible to edit or delete a comment once it has
been made. This endpoint requires ODK Central v1.2 or later.

## See also

<https://docs.getodk.org/central-api-submission-management/#posting-comments>
