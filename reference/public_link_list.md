...
A tibble with one row per Public Access Link and all Link metadata as
columns as per the ODK Central API docs. Column names are renamed from
ODK Central's `camelCase` to `snake_case`.

## Details

This lists every Public Access Link upon the given Form. Anybody in
possession of a Public Access Link for a Form can use that link to
submit data to that Form.

## See also

<https://docs.getodk.org/central-api-form-management/#listing-all-links>
