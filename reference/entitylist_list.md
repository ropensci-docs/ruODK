...
- deleted:
  ...
  (lgl) If `TRUE`, list only deleted Entity Lists with their numeric
  IDs. Default: `FALSE`.
...
A tibble with exactly one row for each Entity List of the given Project
as per ODK Central API docs. Column names are renamed from ODK Central's
`camelCase` to `snake_case`. With `deleted = TRUE`, the tibble holds one
row per deleted Entity List with its numeric ID.

## Details

The returned list is useful to retrieve the valid name of an Entity List
for further use by functions of the Entity Management family.

With `deleted = TRUE`, only deleted Entity Lists with their numeric IDs
are returned, which can be used to export a deleted Entity List with
