...
- display_name:
  ...
  (character) The friendly display name to associate with this User.
  Default: `NULL` (left unchanged).

- email:

  (character) The email address to associate with this User. Default:
  `NULL` (left unchanged).
...
JSON data can be patched to update User details. The display name sets
the friendly name the web interface uses to refer to the User. The email
sets the email address associated with the account. When User details
are updated, the updatedAt field is updated automatically.

## See also

<https://docs.getodk.org/central-api-accounts-and-users/#modifying-a-user>
