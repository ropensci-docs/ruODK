...
- state:
  ...
  (character) The new lifecycle state of the Form. One of `"open"`,
  `"closing"` or `"closed"`.
...
A list with the updated Form's metadata as per the ODK Central API docs.
Top level list elements are renamed from ODK's `camelCase` to
`snake_case`.

## Details

It is currently possible to modify only one thing about a Form: its
state, which governs whether it is available for download onto survey
clients and whether it accepts new Submissions.

Only the properties you supply are changed. Anything you do not supply
remains untouched. This changes only the `state`. It does not change
other properties.

## See also

<https://docs.getodk.org/central-api-form-management/#modifying-a-form>
