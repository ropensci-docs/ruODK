...
A tibble with one row per expected Form attachment and the columns
`name`, `type`, `exists`, `blob_exists`, `dataset_exists`, `size`,
`hash` and `updated_at`, as far as Central returns them. Column names
are renamed from ODK Central's `camelCase` to `snake_case`.

## Details

This endpoint fetches the list of expected attachment files, and tells
whether the server holds each file or not, and a hash of the file
contents. To modify an attachment, create a Draft. The property
`datasetExists` tells whether the attachment is a link to a Dataset
