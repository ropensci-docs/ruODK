...
- `label_lang` The field label in language *\_lang* as given in the form
  schema.

- `hint` The field hint as given in the form schema. If specific
  languages are available, this column will return the `default`
  language or it will be empty if this is not specified.

- `hint_lang` The field hint in language *\_lang* as given in the form
  schema.
...
`hints` and `choice_lists`.

`form_schema_ext` returns the same object as
[`form_schema`](https://docs.ropensci.org/ruODK/reference/form_schema.md)
adding `labels`, `hints` and `choice lists` in all languages available.
This is done by using the return object from
