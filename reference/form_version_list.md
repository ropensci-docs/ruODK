...
A tibble with one row per published Form version and all version
metadata as columns as per the ODK Central API docs. Column names are
renamed from ODK Central's `camelCase` to `snake_case`.

## Details

Each entry of the version listing contains some of the same duplicate
keys with basic information about the Form: `xmlFormId` and `createdAt`,
for example. This matches the data received when each version is
requested separately.

## See also

<https://docs.getodk.org/central-api-form-management/#listing-published-form-versions>
