...
A DELETE to a Link resource removes the Link from the system entirely.
To only revoke the Link's access and prevent future Submissions without
removing its record, terminate its session token instead.

## See also

<https://docs.getodk.org/central-api-form-management/#deleting-a-link>
