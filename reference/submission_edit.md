...
- field:
  ...
  (character) The XForms field to edit, e.g. `"name"` or `"/data/name"`.

- value:

  The new field value, coerced to a string. It must match the field type
  from
  [`form_schema()`](https://docs.ropensci.org/ruODK/reference/form_schema.md):
  integers for `"int"`, numbers for `"decimal"`, anything for
  `"string"`.

- comment:

  (character) A comment about the edit, posted with
...
A list with the `update` result from
[`submission_update()`](https://docs.ropensci.org/ruODK/reference/submission_update.md)
and, if a comment was given, the `comment` result from
[`submission_comment_create()`](https://docs.ropensci.org/ruODK/reference/submission_comment_create.md).

## Details

This convenience wrapper edits one field of a Submission and,
optionally, leaves a comment about the edit. It downloads the current
Submission XML, sets the field to the new value, rotates the
`deprecatedID` metadata node to the current `instanceID`, assigns a new
`instanceID`, uploads the result with
[`submission_update()`](https://docs.ropensci.org/ruODK/reference/submission_update.md),
and posts the comment with
[`submission_comment_create()`](https://docs.ropensci.org/ruODK/reference/submission_comment_create.md).
The new value is checked against the field type from
[`form_schema()`](https://docs.ropensci.org/ruODK/reference/form_schema.md).
