...
A tibble with one row per Assignment and the columns `actor_id` and
`role_id`.

## Details

This lists every Assignment upon the given Project in the form of
`actor_id`/`role_id` pairs. Assigning an Actor a Role grants that Actor
the Role's verbs anywhere within the Project.

## See also

<https://docs.getodk.org/central-api-project-management/#listing-all-project-assignments>
