...
When a Form is deleted, it goes into the Trash section, but it can now
be restored from the Trash with
...
Submissions will be automatically purged. If the goal is to prevent the
Form from showing up on survey clients like ODK Collect, consider
setting its state to closing or closed instead (see
[`form_update()`](https://docs.ropensci.org/ruODK/reference/form_update.md)).

## See also

<https://docs.getodk.org/central-api-form-management/#deleting-a-form>
