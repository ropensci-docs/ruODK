...
A list with one entry per affected Dataset, each holding the Dataset
`name`, its `isNew` flag, and its `properties` with `inForm` and `isNew`
flags.

## Details

This endpoint reflects the change to a Dataset that takes effect once
the Form is published. Like the published endpoint, it lists the Dataset
name and Properties, but it also includes the `isNew` flag on both the
Dataset and each individual Property. This flag is true only if the
Dataset or Property is new and publishing the Draft Form creates it.

## See also

<https://docs.getodk.org/central-api-form-management/#draft-form-dataset-diff>
